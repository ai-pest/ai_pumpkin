...
def get_image_binary(asset):
    image_base64 = asset["images"][0]["data"]

    # MIME ヘッダがあれば削除
    mime_jpeg = "data:image/jpeg;base64,"
    if image_base64.startswith(mime_jpeg):
        image_base64 = image_base64[len(mime_jpeg):]
...
def get_plant(asset):
    ...
    '''
    # attributes がなければ、作物でフィルタリングしない
    # （NOTE: カボチャうどんこ病システムの中継サーバは attributes を付加しない）
    try:
...
def _application(environ, start_response):
    ...
    try:
        ...
        for reqAsset in reqData["assets"]:
            ...
            image_b = get_image_binary(reqAsset)
